...
function writeStoredSidebarWidth(key: string, width: number) {
	try {
...
const SidebarInstanceContext =
	createContext<SidebarInstanceContextProps | null>(null);
	...
	const setOpen = useCallback(
		async (value: boolean | ((value: boolean) => boolean)) => {
			...
			try {
				...
				// Cookie Store API unavailable in some contexts.
			}
		},
		[setOpenProp, open]
	);
...
function Sidebar({
	...
	const resolvedResizable =
		useMemo<SidebarResolvedResizableOptions | null>(() => {
	...
	const instanceContextValue = useMemo<SidebarInstanceContextProps>(
			...
			<SidebarInstanceContext value={instanceContextValue}>
				<Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
					...
					>
						...
						<div
							className={cn(
								"flex h-full w-full flex-col pt-safe pb-safe",
...
function SidebarRail({
	...
	const sidebarInstance = use(SidebarInstanceContext);
	...
	useEffect(
		() => () => {
			...
			if (resizeState?.transitionTargets) {
				for (const element of resizeState.transitionTargets) {
					element.style.removeProperty("transition-duration");
				}
			}
				...
				render={
					<button
						...
						className={cn(
							...
							"[[data-side=left][data-state=expanded]_&]:right-auto [[data-side=left][data-state=expanded]_&]:left-full [[data-side=left][data-state=expanded]_&]:translate-x-0 [[data-side=left][data-state=expanded]_&]:after:left-0",
							"[[data-side=right][data-state=expanded]_&]:right-full [[data-side=right][data-state=expanded]_&]:left-auto [[data-side=right][data-state=expanded]_&]:translate-x-0 [[data-side=right][data-state=expanded]_&]:after:right-0",
							/* Collapsed: keep the rail centered on the panel edge for toggle affordance. */
							"[[data-side=left][data-state=collapsed]_&]:-right-4 [[data-side=left][data-state=collapsed]_&]:-translate-x-1/2 [[data-side=left][data-state=collapsed]_&]:after:left-1/2",
							"[[data-side=right][data-state=collapsed]_&]:left-0 [[data-side=right][data-state=collapsed]_&]:-translate-x-1/2 [[data-side=right][data-state=collapsed]_&]:after:left-1/2",
...
function SidebarHeader({ className, ...props }: ComponentProps<"div">) {
	return (
		<div
			className={cn("relative z-10 flex flex-col gap-2 p-2", className)}
			data-sidebar="header"
...
function SidebarContent({ className, ...props }: ComponentProps<"div">) {
	return (
		<ScrollArea className="h-auto min-h-0 flex-1" hideScrollbars scrollFade>
			<div
				className={cn(
					"relative z-10 flex w-full min-w-0 flex-col gap-2 group-data-[collapsible=icon]:overflow-hidden",
					className
				)}
...
function SidebarMenuSub({ className, ...props }: ComponentProps<"ul">) {
	return (
		<ul
			className={cn(
				"ml-3 flex min-w-0 translate-x-px flex-col gap-1 border-sidebar-border border-l py-0.5 pr-1 pl-2.5",
				"group-data-[collapsible=icon]:hidden",
				className
			)}
