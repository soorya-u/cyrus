...
Cross-platform React providers for query client, signaling, and worker RTC connections, with type-safe internal consumption in `@cyrus/hooks`, lifecycle cleanup, and error/retry UX.
...
The system SHALL provide `QueryProvider` in `@cyrus/providers` with an `onError` callback, replacing duplicated web and mobile query-client configuration.

#### Scenario: Platform-specific error handling

- **WHEN** web mounts `QueryProvider` with an `onError` callback that shows a toast
...
- **WHEN** mobile mounts `QueryProvider` with an `onError` callback that logs via evlog
- **THEN** query errors are handled by that callback using the same shared provider
...
Internal context consumption hooks in `@cyrus/hooks/src/contexts/` (`useSignaling`, `useRtc`) SHALL return non-optional types when called inside the correct provider tree. These hooks SHALL NOT be exported from the `@cyrus/hooks` package.

#### Scenario: Internal hook called inside provider

- **WHEN** internal `useRtc()` is called inside `RtcProvider` after a successful connection
...
Shared data hooks that depend on controller or signaling RPC SHALL consume internal context hooks, not router context.
